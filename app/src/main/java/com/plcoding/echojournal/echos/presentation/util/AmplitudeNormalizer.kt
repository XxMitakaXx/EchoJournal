...
object AmplitudeNormalizer {
    ...
    private const val AMPLITUDE_MIT_OUTPUT_THRESHOLD = 0.1f
    ...
    private fun remapAmplitudes(amplitudes: List<Float>): List<Float> {
        ...
        val scaleFactor = MAX_OUTPUT - AMPLITUDE_MIT_OUTPUT_THRESHOLD
        return amplitudes.map { amplitude ->
            if (amplitude <= AMPLITUDE_MIT_OUTPUT_THRESHOLD) {
                MIN_OUTPUT
            } else {
                val amplitudeRange = amplitude - AMPLITUDE_MIT_OUTPUT_THRESHOLD
    ...
    // [0, 0.1, 0] -> [0, 0.25, 0.05, 0.075, 1, 0.075, 0.05, 0.025, 0]
    private fun upsample(source: List<Float>, targetSize: Int): List<Float> {
        ...
        for (i in 0 until targetSize) {
            ...
            // How far we are pas that item as percentage of the gap to the next one
